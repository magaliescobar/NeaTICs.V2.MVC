...
namespace NeaTICs_v2.Helpers
{
    public static class CultureHelper
    {
        ...
        /// <summary>
        /// Devuelve un nombre de referencia cultural válido basado en el parámetro "name". Si "name" no es válido, devuelve la referencia cultural predeterminada "es"
        /// </summary>
        /// <param name="name" />El nombre de la cultura (por ejemplo, es)</param>
        public static string GetImplementedCulture(string name)
        {
            // Nos aseguramos de que no es nulo
            if (string.IsNullOrEmpty(name))
                return GetDefaultCulture(); // Retornar la cultura por defecto
            // Nos aseguramos que en primer lugar este una referencia cultural válida
            if (_validCultures.Where(c => c.Equals(name, StringComparison.InvariantCultureIgnoreCase)).Count() == 0)
                return GetDefaultCulture(); // Retornamos la cultura por defecto en caso de que sea inválida 
            // Si está implementada, lo aceptamos
            if (_cultures.Where(c => c.Equals(name, StringComparison.InvariantCultureIgnoreCase)).Count() > 0)
                return name; // Aceptamos
        ...
        /// <summary>
        /// Retornamos el nombre predeterminado de la cultura, que es el primer nombre declarado (por ejemplo, es)
        /// </summary>
        /// <returns></returns>
        public static string GetDefaultCulture()
        {
            return _cultures[0]; // Retornamos la cultura por defecto
        }
        ...
        public static string GetNeutralCulture(string name)
        {
            if (name.Length < 2)
                return name;
            return name.Substring(0, 2); // Leemos los primero dos únicos caracteres. Por ejemplo "es", "es"
        }
    }
}